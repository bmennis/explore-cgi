...
def mk_fa_seq(mat_file, outFile):
    with open(mat_file, 'r') as inf, open(outFile, 'w') as of:
        reader = csv.DictReader(inf, delimiter='\t')
#        allele_freq = float(int(outFile.split('/')[-1].split('.')[-2])/100)
        for row in reader:
#            if float(row['af']) >= allele_freq:
            if row['alt'][0] == row['ref'][0]:
                if abs(int(row['indel_length'])) < 50:
                    if int(row['indel_length']) > 0 and int(row['indel_length']) < 50:
                    ...
                    elif int(row['indel_length']) < 0 and int(row['indel_length']) > -50:
