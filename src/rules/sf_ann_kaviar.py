...
rule intersectFilesAll:
    input: expand(DATA + 'interim/kaviar_subsets/{subset}/{subset}_sql_result.status.intr', subset=('cgi_only_sources','illumina_only_sources_filtered','cgi_illumina_sources'))
...
rule intersectDifficultByFile:
    ...
    output: DATA + 'interim/{subset}/{subset}_{subset2}__{subset3}.intr'
    shell: 'bedtools intersect -wb -a {input.vcf} -b {input.bed} > {output}'

rule intersectAllDifficultByFile:
    input: expand(DATA + 'interim/{subset}/{subset}_{subset4}.intr', \
                  subset=('cgi_only_sources','illumina_only_sources_filtered','cgi_illumina_sources'), \
